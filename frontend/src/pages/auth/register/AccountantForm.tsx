...
export function AccountantForm({ meta, onSuccess }: { meta: RegistrationMeta; onSuccess: (r: RegistrationResult) => void }) {
  ...
  async function onSubmit(values: FormValues) {
    setServerError(null);
    try {
      const result = await submitRegistration("accountant", {
  ...
  return (
    <form className="space-y-1" onSubmit={handleSubmit(onSubmit)} noValidate>
      <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Accountant registration</h2>
